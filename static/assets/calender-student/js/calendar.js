var cal = {
  ...
  list : () => {
    ...
    for (let i=0; i<total; i++) {
      ...
      if (squares[i]=="b") { cCell.classList.add("blank"); }
        ...
        var lseg = urllist.split("/").pop();
        var text1 = "/studentcalendarjson/";
        var urldata = text1.concat(lseg);
         fetch(urldata).then(function(response) {
           return response.json();
         }).then(function(parsedJson) {
            for(var i=0;i<Object.keys(parsedJson).length;i++)
            {
              if(Object.keys(parsedJson)[i].length==8)
              {
                    if((cCell.textContent==Object.keys(parsedJson)[i].slice(0,1) && (cal.sMth+1)==Object.keys(parsedJson)[i].slice(2,3) && cal.sYear==Object.keys(parsedJson)[i].slice(4,9))===true)
                    {
                      ...
                      cCell.innerHTML += "<div" + " name="+ `"` + datastore + `"` + "class='evt'>" + datastore + "</div>";
                    }
                      //console.log(cCell);
                      cal.show(cCell);
                      }
                     else if(Object.keys(parsedJson)[i].length==9 && (cal.sMth+1)<10)
                     {
                       if((cCell.textContent==Object.keys(parsedJson)[i].slice(0,2) && (cal.sMth+1)==Object.keys(parsedJson)[i].slice(3,4) && cal.sYear==Object.keys(parsedJson)[i].slice(5,9))===true)
                       {
                           datastore=parsedJson[Object.keys(parsedJson)[i]];
                           cal.data[cal.sDay]=datastore;
                           cal.hfTxt.value=true;
                        ...
                        else if(Object.keys(parsedJson)[i].length==9 && (cal.sMth)>10)
                        {
                          if((cCell.textContent==Object.keys(parsedJson)[i].slice(0,1) && (cal.sMth+1)==Object.keys(parsedJson)[i].slice(2,4) && cal.sYear==Object.keys(parsedJson)[i].slice(5,9))===true)
                          {
                        ...
                        {
                          if((cCell.textContent==Object.keys(parsedJson)[i].slice(0,2) && (cal.sMth+1)==Object.keys(parsedJson)[i].slice(3,5) && cal.sYear==Object.keys(parsedJson)[i].slice(6,10))===true)
                          {
                              datastore=parsedJson[Object.keys(parsedJson)[i]];
                              cal.data[cal.sDay]=datastore;
                              cal.hfTxt.value=true;
  ...
  save : () => {
    ...
    // console.log(cal.sDay);
    // console.log(cal.sYear);
    // console.log(cal.sMth+1);
...
//    console.log(url)
    ...
    document.getElementById(date1).parentElement.innerHTML += "<div" + " name="+ `"` + valuing + `"` + "class='evt'>" + valuing + "</div>";//---------->Buggy
  //   var fs = require('fs');
